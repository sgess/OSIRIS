function WRITE_OS(myfilein, myfileout, os_struct)
...
while( row ~= -1 )
  ...
  if( row ~= -1 )
    ...
    elseif(~isempty(strfind(row, 'coordinates')) && section_grid == 1)
       fprintf(fidout, ['  coordinates = ' os_struct.grid.coordinates]);
    ...
    elseif(~isempty(strfind(row, 'xmin')) && section_space == 1)
       fprintf(fidout, ['  xmin(1:2) = ' os_struct.space.xmin]);
    elseif(~isempty(strfind(row, 'xmax')) && section_space == 1)
       fprintf(fidout, ['  xmax(1:2) = ' os_struct.space.xmax]);
