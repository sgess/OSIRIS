...
#$ -cwd
#  input           = /dev/null
#  output          = /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog
#$ -o /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID
#  error           = Merged with joblog
#$ -j y
#  The following items pertain to the user program
#  user program    = /u/scratch/$fl/$me/$dirname/osiris-2D.e
#  arguments       = 
...
#
  ...
  set qqptasks  = $nodes
  set qqidir    = /u/scratch/$fl/$me/$dirname
  set qqjob     = osiris-2D.e
  set qqodir    = /u/scratch/$fl/$me/$dirname
  cd     /u/scratch/$fl/$me/$dirname
...
#
  ...
  time \$MPI_BIN/mpiexec -n $nodes -machinefile \$QQ_NODEFILE  \\
         /u/scratch/$fl/$me/$dirname/osiris-2D.e  >& /u/scratch/$fl/$me/$dirname/osiris-2D.e.output.\$JOB_ID 
...
#
  source /u/local/bin/qq.sge/qr.runtime
#
  echo "-------- /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID --------" >> /u/local/apps/queue.logs/openmpi.log.parallel
 if (\`wc -l /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID  | awk '{print \$1}'\` >= 1000) then
        head -50 /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID >> /u/local/apps/queue.logs/openmpi.log.parallel
        echo " "  >> /u/local/apps/queue.logs/openmpi.log.parallel
        tail -10 /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID >> /u/local/apps/queue.logs/openmpi.log.parallel
  else
        cat /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID >> /u/local/apps/queue.logs/openmpi.log.parallel
  endif
#  cat            /u/scratch/$fl/$me/$dirname/osiris-2D.e.joblog.\$JOB_ID           >> /u/local/apps/queue.logs/openmpi.log.parallel
  exit (0)
EOF
